...
function App() {
  ...
  const calculateFare = useCallback((distanceKm: number, waitingMinutes: number, sorianaBonus: boolean = false) => {
    // Solo agregar $5 MXN si Soriana está activo Y el recorrido es 3.7km o más
    const sorianaExtraFee = (sorianaBonus && distanceKm >= 3.7) ? 5 : 0;
    ...
    if (selectedTripType.fixedPrice || (selectedTripType.id === 'cristoRey' && selectedSubTrip)) {
      ...
      if (distanceKm > baseDistance) {
        ...
        if (selectedTripType.id === 'cristoRey') {
          // Cristo Rey: Lógica especial de incrementos
          if (distanceKm > 3.5) {
            // Si supera 3.5km, aumentar $20 MXN
            baseCost += 20;
          } else if (distanceKm > 2.0) {
            // Si supera 2.0km pero no 3.5km, aumentar $10 MXN
            baseCost += 10;
          }
          // No hay costo adicional si está entre 0-2.0km
        } else {
          // Lógica normal para otros destinos fijos
          const extraCost = extraKm * 16; // $16 por km adicional (igual que tarifa normal)
          baseCost += extraCost;
        }
      }
      
      const waitingCost = waitingMinutes * RATES.waitingRate;
      return baseCost + waitingCost + sorianaExtraFee + petExtraFee;
    }
    ...
    return fare + (waitingMinutes * RATES.waitingRate) + sorianaExtraFee + petExtraFee;
  }, [selectedTripType, selectedSubTrip, petConfig]);
  ...
  const startTestMode = () => {
    ...
    testModeInterval.current = setInterval(() => {
      // Incremento más controlado para simular movimiento realista
      // Aproximadamente 0.5km por segundo en modo prueba
      simulatedLat += (Math.random() - 0.5) * 0.005;
      simulatedLng += (Math.random() - 0.5) * 0.005;
          ...
          {!tripData.isRunning && (
            <div className="mb-4">
              ...
              {(selectedTripType.fixedPrice || selectedSubTrip) && (
                <div className="mt-2 bg-gradient-to-r from-green-800 to-green-900 text-white p-3 rounded-xl text-center border border-green-500 shadow-lg">
                  <p className="text-sm">
                    ✅ Bonus Soriana activo: +$5 MXN (si ≥3.7km)
                  </p>
                  {(selectedTripType.fixedPrice || selectedSubTrip) && (
                  ...
                  <p className="text-green-300 text-xs mt-1">
                    Ejemplo: 3.9km = $50 + $5 MXN extra = $55 MXN
                  </p>
                </div>
              )}
              ...
              <div className="mt-3">
                <label className="flex items-center bg-gradient-to-r from-orange-600 to-orange-700 hover:from-orange-700 hover:to-orange-800 text-white p-4 rounded-xl cursor-pointer transition-all border border-orange-500 shadow-lg">
                  ...
                  />
                  <div className="flex-1">
                    <div className="font-bold text-lg">🏪 Saliendo de Soriana</div>
                    <div className="text-sm text-orange-200 mt-1">
                      Agrega +$5 MXN al costo si el recorrido es ≥ 3.7km
                    </div>
                  </div>
                </label>
                
                {isSorianaActive && (
                  <div className="mt-2 bg-gradient-to-r from-orange-800 to-orange-900 text-white p-3 rounded-xl text-center border border-orange-500 shadow-lg">
                    <p className="text-sm">
                      ✅ Bonus Soriana activo: +$5 MXN se agregará si ≥ 3.7km
                    </p>
                    <p className="text-orange-300 font-bold mt-1 text-xs">
                      Ejemplo: 3.9km = tarifa normal + $5 MXN extra
                    </p>
                  </div>
                )}
              </div>
                ...
                <div className="flex items-center">
                  ...
                  <div className="text-left">
                    ...
                    <div className="text-sm text-gray-300">
                      {serviciosExtrasActive || petConfig.active 
                        ? `${serviciosExtrasActive ? 'Servicios Extras' : ''}${serviciosExtrasActive && petConfig.active ? ', ' : ''}${petConfig.active ? `Mascotas ${petConfig.withCage ? '(con jaula)' : '(sin jaula)'}` : ''}` 
          ...
          {tripData.isRunning && currentPosition && (
            <div className="mt-4 bg-gradient-to-r from-purple-800 to-purple-900 text-white p-3 rounded-xl text-center border border-purple-500 shadow-lg">
              <div className="text-xs">
                ...
                <div>Distancia GPS: {tripData.rawDistance.toFixed(3)} km</div>
                <div>Distancia ajustada: {tripData.distance.toFixed(3)} km</div>
                <div>Descuento aplicado: {((tripData.rawDistance - tripData.distance) * 1000).toFixed(0)} m</div>
                <div>Soriana bonus: {isSorianaActive ? (tripData.distance >= 3.7 ? '+$5 aplicado' : 'Pendiente (≥3.7km)') : 'Inactivo'}</div>
                <div>Mascotas: {petConfig.active ? `+$${petConfig.cost} (${petConfig.withCage ? 'con jaula' : 'sin jaula'})` : 'Inactivo'}</div>
